...
| (statusnya COMPLETED), jadi kalau tidak kita tahan secara lokal, pesanan
| ini akan langsung hilang dari tabel Pesanan Pending begitu dibayar --
| padahal user belum sempat cetak struk.
|
| Setelah struk berhasil dicetak, entry-nya dihapus dari sini (order tetap
| ada normal di History).
|--------------------------------------------------------------------------
*/
...
export async function addAwaitingReceipt(order: Order) {
  ...
  if (list.some((item) => item.order.id === order.id)) return;
